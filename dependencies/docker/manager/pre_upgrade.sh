...
then
    ...
    do
        ...
        then
            ...
            echo "start to import data into $database from $DUMP_FILE"
            mysqldump --single-transaction  --set-gtid-purged=OFF --host=$host --port=$port --user=$user --password=$password $database > $DUMP_FILE
            chmod 700 "$DUMP_FILE"
            echo "import data into $database successfully"
        fi
    done
else
